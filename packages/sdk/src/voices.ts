 ...
 */
export const NEURAL_VOICE_LIBRARY: CertifiedVoice[] = [
    // ═══════════════════════════════════════════════════════════════════
 ...
 */
export function getVoicesByCategory(category: VoiceCategory): CertifiedVoice[] {
    return NEURAL_VOICE_LIBRARY.filter(v => v.category === category);
}

/**
 * Get all categories with voice counts
 */
export function getVoiceCategories(): Array<{ category: VoiceCategory; count: number }> {
    const counts = new Map<VoiceCategory, number>();
    NEURAL_VOICE_LIBRARY.forEach(v => {
 ...
 * Find a voice by ID
 */
export function findVoiceById(id: string): CertifiedVoice | undefined {
    return NEURAL_VOICE_LIBRARY.find(v => v.id === id);
}

/**
 * Get a random voice from a category
 */
export function getRandomVoice(category?: VoiceCategory): CertifiedVoice {
    const pool = category
        ? NEURAL_VOICE_LIBRARY.filter(v => v.category === category)
