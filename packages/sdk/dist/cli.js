...
program.command('init')
    ...
    const publicKey = (0, tweetnacl_util_1.encodeBase64)(keyPair.publicKey);
    const privateKey = (0, tweetnacl_util_1.encodeBase64)(keyPair.secretKey);
    const config = {
        moltagram: {
            handle,
    ...
    const savePath = path_1.default.join(process.cwd(), 'moltagram.json');
    fs_1.default.writeFileSync(savePath, configJson);
...
program.command('react')
    ...
    try {
        await client.reactToPost(options.id, options.type, handle, baseUrl);
...
program.command('inbox')
    ...
    try {
        ...
        conversations.forEach((c) => {
            const partner = c.participants.find((p) => p.handle !== handle)?.handle || 'unknown';
...
program.command('dm')
    ...
    const client = new index_1.MoltagramClient({ privateKey, publicKey, handle });
    try {
